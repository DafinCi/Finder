...
describe(
  "Integration: Sui Account Linking & Unlinking (Phase 5)",
  { timeout: 30000 },
  () => {
    ...
    function buildLinkReq(
      ...
      const ip =
        options.ip ||
        `test-ip-${Date.now()}-${Math.random().toString(36).substring(7)}`;
    ...
    it("should reject linking a wallet that already belongs to another user with 409 WALLET_ALREADY_LINKED", async () => {
      ...
      const { data: profileX } = await admin
        ...
        .eq("id", userX.userId)
        .single();
      expect(profileX?.sui_address).toBe(
        normalizeSuiAddress(keypairA.toSuiAddress()),
      );
    });
    ...
    it("should reject unlink for Sui-only user with 400 NO_ALTERNATIVE_AUTH_METHOD", async () => {
      ...
      const { data: profile } = await admin
        ...
        .eq("id", resolved.userId)
        .single();
      expect(profile?.sui_address).toBe(
    ...
    it("should NOT delete the user record from auth.users upon unlinking", async () => {
      ...
      const { data: checkUser, error: checkError } =
        await admin.auth.admin.getUserById(user.userId);
    ...
    it("should allow another user to link the old wallet after it was unlinked", async () => {
      ...
      const { data: profileB } = await admin
        ...
        .eq("id", userB.userId)
        .single();
      expect(profileB?.sui_address).toBe(
        normalizeSuiAddress(keypair.toSuiAddress()),
      );
    });
    ...
    it("should preserve RLS boundary: user can only access own profile before, during, and after link", async () => {
      ...
      // 1. Initial RLS check
      const { data: ownInit } = await clientA
        .from("profiles")
        .select("*")
        .eq("id", userA.userId)
        .single();
      ...
      const { data: otherInit } = await clientA
        .from("profiles")
        .select("*")
        .eq("id", userB.userId)
        .maybeSingle();
      ...
      const { data: ownLinked } = await clientA
        .from("profiles")
        .select("*")
        .eq("id", userA.userId)
        .single();
      expect(ownLinked?.sui_address).toBe(
        normalizeSuiAddress(keypair.toSuiAddress()),
      );

      const { data: otherLinked } = await clientA
        .from("profiles")
        .select("*")
        .eq("id", userB.userId)
        .maybeSingle();
      ...
      const { data: ownUnlinked } = await clientA
        .from("profiles")
        .select("*")
        .eq("id", userA.userId)
        .single();
      ...
      const { data: otherUnlinked } = await clientA
        .from("profiles")
        .select("*")
        .eq("id", userB.userId)
        .maybeSingle();

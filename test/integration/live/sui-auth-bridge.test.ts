...
describe(
  "Integration: Sui SIWS & Supabase Session Bridge",
  { timeout: 20000 },
  () => {
    ...
    it("TEST E — Profile trigger behavior on auth.users creation", async () => {
      ...
      expect(profileRows?.[0].sui_address).toBe(
    ...
    it("API Endpoints: /api/auth/sui/nonce and /api/auth/sui/verify e2e flow", async () => {
      ...
      const nonceReq = new NextRequest(
        "http://localhost:3000/api/auth/sui/nonce?purpose=SIWS_LOGIN",
        {
      ...
      const verifyReq = new NextRequest(
        "http://localhost:3000/api/auth/sui/verify",
        {
          method: "POST",
          headers: {
      ...
      expect(verifyResult.user.suiAddress).toBe(
        normalizeSuiAddress(suiAddress),
      );
